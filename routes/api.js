...
const listkey = ["supraz", "yunaBot", "lhannaBot", "newtonCant"];
var len = 15
...
router.get('/covidbr', async (req, res, next) => {
        var Apikey = req.query.apikey;
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
       fetch(encodeURI(`https://api-gdr2.herokuapp.com/api/covidbr`))
        .then(response => response.json())
        .then(data => {
        var pais = data.result.local;
        var tolcasos = data.result.totalCasos;
        var novcasos = data.result.novosCasos;
        var tolmortos = data.result.totalMortes;
        var novmortos = data.result.novasMortes;
        var recupe = data.result.recuperados;
        var vacinados = data.result.vacinadosPrimeiraDose;
             res.json({
                 criador : `${creator}`,
                 pais,
                 tolcasos,
                 ...
                 recupe,
                 vacinados,
                 fotolink : `https://i1.wp.com/jornaltribunalivre.com/wp-content/uploads/2020/11/Covid-5.jpg?w=626&ssl=1`
             })
         })
         } else {
res.json(loghandler.invalidKey)
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
       fetch(encodeURI(`https://docs-jojo.herokuapp.com/api/translate?text=${text}&from=id&to=pt`))
        .then(response => response.json())
        .then(data => {
        var original = data.original_text;
         var traduzido = data.translated_text;
             res.json({
                 criador : `${creator}`,
                 original,
                 traduzido
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
       ...
       fetch(encodeURI(`https://bx-hunter.herokuapp.com/api/calculator?angka=${cul}&apikey=Ikyy69`))
        .then(response => response.json())
        .then(data => {
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
       ...
       fetch(encodeURI(`https://brasilapi.com.br/api/ddd/v1/${quero}`))
        .then(response => response.json())
        .then(data => {
        var estado = data.state;
        var cidades = data.cities;
             res.json({
                 criador : `${creator}`,
                 estado,
                 cidades
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
       fetch(encodeURI(`https://brasilapi.com.br/api/cep/v1/${quero}`))
        .then(response => response.json())
        .then(data => {
         var cep = data.cep;
        var estado = data.state;
        var cidade = data.city;
       var lugar = data.neighborhood; 
       var rua = data.street;
             res.json({
                 criador : `${creator}`,
                 cep,
...
router.get('/youtpes', async (req, res, next) => {
         quero = req.query.quero
        var Apikey = req.query.apikey;
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
       fetch(encodeURI(`https://api-gdr2.herokuapp.com/api/yt/search?q=${quero}`))
        .then(response => response.json())
        .then(data => {
         var titulo = data.title;
        var duracao = data.duration;
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
       fetch(encodeURI(`http://ip-api.com/json/${quero}`))
        .then(response => response.json())
        .then(data => {
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
  dataf = await fetchJson(`https://pastebin.com/raw/cVDj0qz6`)
      casas = JSON.parse(JSON.stringify(dataf));
      afs = casas[Math.floor(Math.random() * casas.length)]; 
        var resultado = afs.result;
             res.json({
                 criador : `${creator}`,
                 resultado
             })       
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
  dataf = await fetchJson(`https://pastebin.com/raw/Xb79iU93`)
      casas = JSON.parse(JSON.stringify(dataf));
      afs = casas[Math.floor(Math.random() * casas.length)]; 
        var frase = afs.teks;
             res.json({
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
  dataz = await fetchJson(`https://pastebin.com/raw/RvXdU6eu`)
      casar = JSON.parse(JSON.stringify(dataz));
      afsi = casar[Math.floor(Math.random() * casar.length)]; 
        var frase = afsi.frase;
             res.json({
                 criador : `${creator}`,
                 frase
             })      
...
router.get('/memebr', async (req, res, next) => {         
        var Apikey = req.query.apikey;
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
     dataf = await fetchJson(`https://dapuhy-api.herokuapp.com/api/search/googleimage?query=meme%20br&apikey=YwrxLyeC8lv8yhI`)
    var imagem = dataf.image
	       res.json({
                 criador : `${creator}`,
                 imagem
             })     
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){        
     dataf = await fetchJson(`https://some-random-api.ml/animu/hug`)
    var link = dataf.link;
	       res.json({
                 criador : `${creator}`,
                 link
...
router.get('/semoji2', async (req, res, next) => {       
        text2 = req.query.text2
      var apikeyInput = req.query.apikey      
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'supra') return res.json(loghandler.invalidKey)     
     dataf = await fetchJson(`https://bx-hunter.herokuapp.com/api/maker/emoji2png?text=${text2}&apikey=Ikyy69`)
...
router.get('/frases', async (req, res, next) => {
     var apikeyInput = req.query.apikey      
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'supra') return res.json(loghandler.invalidKey)
 var datag = fs.readFileSync('./apis/quote.js');
    var qarr = JSON.parse(JSON.stringify(datag));
    afss = qarr[Math.floor(Math.random() * qarr.length)]; 
var resultados = afss.result;
             res.json({
                 criador : `${creator}`,
                 resultados
             })         	
...
if(!Apikey) return res.json(loghandler.notparam)
   ...
   hasill = await fs.readFileSync(__path +'/apis/' + random + '.webp');
  
         await fs.writeFileSync(__path +'/tmp/dadu.webp', hasill)
...
if(!Apikey) return res.json(loghandler.notparam)
   ...
   hasill = await fs.readFileSync(__path +'/stickerr/' + random + '.webp');
  
         await fs.writeFileSync(__path +'/tmp/stickera.webp', hasill)
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
   hasil = 'https://www.myinstants.com/media/sounds/' + quero + '.mp3'
...
router.get('/hentaigif', async (req, res, next) => {
     var Apikey = req.query.apikey;
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
   hasil = 'https://dapuhy-api.herokuapp.com/api/nsfw/nsfwhentaigif?apikey=p0h6z3nleqWWAmg'
	  data = await fetch(hasil).then(v => v.buffer())   
  
         await fs.writeFileSync(__path +'/tmp/hentaigif.gif',data)
        res.sendFile(__path+'/tmp/hentaigif.gif')
         } else {
res.json(loghandler.invalidKey)
}    
})
router.get('/canvas/removebg', async (req, res, next) => {
     image = req.query.image
     var Apikey = req.query.apikey;
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
   hasil = 'https://dapuhy-api.herokuapp.com/api/canvas/removebg?img=' + image + '&apikey=nu0b5c7e6I2x7Ay'
	  data = await fetch(hasil).then(v => v.buffer())   
  
         await fs.writeFileSync(__path +'/tmp/hent.jpeg',data)
        res.sendFile(__path+'/tmp/hent.jpeg')
         } else {
res.json(loghandler.invalidKey)
}    
})
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
   hasil = 'https://pecundang.herokuapp.com/api/attp?teks=' + text + ''
	  data = await fetch(hasil).then(v => v.buffer())   
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
   hasil = 'https://pecundang.herokuapp.com/api/texttopng4?teks=' + text + ''
	  data = await fetch(hasil).then(v => v.buffer())   
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
   hasil = 'https://dapuhy-api.herokuapp.com/api/nsfw/nsfwhentai?apikey=p0h6z3nleqWWAmg'
	  data = await fetch(hasil).then(v => v.buffer())   
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){      
   hasil = 'https://dapuhy-api.herokuapp.com/api/sfw/sfwwaifu?apikey=p0h6z3nleqWWAmg'
	  data = await fetch(hasil).then(v => v.buffer())   
  
         await fs.writeFileSync(__path +'/tmp/waifu.jpeg',data)
        res.sendFile(__path+'/tmp/waifu.jpeg')
        } else {
res.json(loghandler.invalidKey)
}    
})
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){      
   hasil = 'https://dapuhy-api.herokuapp.com/api/canvas/welcome2?name=' + nome + '&discriminator=' + disc + '&member=' + membro + '&gcname=' + gp + '&pp=' + perfil + '&bg=' + fundo + '&apikey=YwrxLyeC8lv8yhI'
	  data = await fetch(hasil).then(v => v.buffer())   
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){  
   hasil = 'https://dapuhy-api.herokuapp.com/api/canvas/triggered?img=' + imagem + '&apikey=YwrxLyeC8lv8yhI'
	  data = await fetch(hasil).then(v => v.buffer())   
	  await fs.writeFileSync(__path +'/tmp/triggered.webp', data)
       res.sendFile(__path+'/tmp/triggered.webp')
        } else {
res.json(loghandler.invalidKey)
}    
})
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){  
   hasil = 'https://api-exteam.herokuapp.com/api/card-spotify?titulo=' + titulo + '&author=' + text + '&album=Supra`z&capa=' + capa + ''
	  data = await fetch(hasil).then(v => v.buffer())   
...
router.get('/canvas/figmeme', async (req, res, next) => {
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){  
   hasil = 'https://dapuhy-api.herokuapp.com/api/maker/memegen?text1=' + text1 + '&text2=' + text2 + '&img=' + img + '&apikey=YwrxLyeC8lv8yhI'
	  data = await fetch(hasil).then(v => v.buffer())   
	  await fs.writeFileSync(__path +'/tmp/img.webp', data)
       res.sendFile(__path+'/tmp/img.webp')
        } else {
res.json(loghandler.invalidKey)
}    
})
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
   hasil = 'https://dapuhy-api.herokuapp.com/api/canvas/goodbye2?name=' + nome + '&discriminator=' + disc + '&member=' + membro + '&gcname=' + gp + '&pp=' + perfil + '&bg=' + fundo + '&apikey=YwrxLyeC8lv8yhI'
	  data = await fetch(hasil).then(v => v.buffer())   
	  await fs.writeFileSync(__path +'/tmp/bye.jpeg', data)
       res.sendFile(__path+'/tmp/bye.jpeg')
  } else {
res.json(loghandler.invalidKey)
}    
})
...
router.get('/playaudio', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
       fetch(encodeURI(`https://jonaz-api-v2.herokuapp.com/play?music=${quero}`))
        .then(response => response.json())
        .then(data => {
        var titulo = data.titulo;
        var duracao = data.duracao;
        var link = data.link;
         var descricao = data.descricao;
        var tamanho = data.tamanho;
         var baixar = data.url_dl; 
             res.json({
                 titulo,
                 duracao,
                 link,
...
router.get('/playvid', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
       fetch(encodeURI(`https://dapuhy-api.herokuapp.com/api/socialmedia/ytplaymp4v2?query=${quero}&apikey=YwrxLyeC8lv8yhI`))
        .then(response => response.json())
        .then(data => {
        var titulo = data.title;
        var img = data.thumb;
        var link_baixar = data.url;
             res.json({
             criador : `${creator}`,
             titulo,
             img,
...
router.get('/playaudio2', async (req, res, next) => {
       quero = req.query.quero
         var Apikey = req.query.apikey           
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
       fetch(encodeURI(`https://dapuhy-api.herokuapp.com/api/socialmedia/ytplaymp3v2?query=${quero}&apikey=YwrxLyeC8lv8yhI`))
        .then(response => response.json())
        .then(data => {
        var titulo = data.title;
        var img = data.thumb;
...
router.get('/igstalk', async (req, res, next) => {
	...
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
       fetch(encodeURI(`https://dapuhy-api.herokuapp.com/api/socialmedia/igstalk?username=${quero}&apikey=p0h6z3nleqWWAmg`))
        .then(response => response.json())
        .then(data => {
        var nome = data.result.username;
        var fullnome = data.result.fullName;
        var perfil = data.result.profilePic;
         var bio = data.result.biography;
        var seguidores = data.result.followers;
        var seguindo = data.result.following;
             res.json({
             criador : `${creator}`,
             nome,
             fullnome,
...
router.get('/nickff', async (req, res, next) => {
	...
	if(listkey.includes(Apikey)){
       ...
       fetch(encodeURI(`http://brizas-api.herokuapp.com/gerador/fancytext?apikey=brizaloka&text=${text}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
